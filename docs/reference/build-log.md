...
### Surplus cascade, Commit 6: conservation coverage — the gate for Commits 7-8 (spec 39)

**Two real defects found, both closed, before either later commit builds on this engine.** The spec's own words: "this is exactly where this project's money bugs have lived... found twice, in different features." True a third time.

**1. The coverage-report flake was a regression from this spec's own Commit 1, not pre-existing noise.** Flagged (and wrongly filed as "pre-existing, unrelated, out of scope") in this spec's Commits 2, 3 and 4's own build-log entries — a different `(threshold, stratum)` cell reading zero each time. Investigated properly here because Commit 6 is explicitly the commit that has to trust this exact sweep. Root cause: `pickFair`'s own "mathematical guarantee" (spec 28 — every registered cell visited at least once every `list.length`/5 calls sharing a key) depends on its `fairBags` persisting ACROSS calls that share a key. Commit 1's own seeding fix (`eda58c7`) made `reseed()` clear `fairBags` on EVERY `randomScenario()` call, including the sweep's own unseeded ones — needed for single-seed reproduction (`randomScenario(1928374650)` must be a pure function of its seed alone), but it silently also reset the bag for every ordinary sweep iteration. A threshold reached behind a nested, low-probability gate (AGE_THRESHOLDS' 25%-gated cohort, one of 11 named ages, one of 5 strata) is stratified AT MOST ONCE per scenario — with a fresh bag every call, that single draw is a plain i.i.d. pick again in every way that matters, exactly the Poisson-variance failure mode spec 28 built `fairBags` to eliminate. Commit 1's own build log claimed this was "confirmed not to weaken... each call still visits every pickFair key at least once via its own fresh shuffle" — true only for a name stratified MULTIPLE times within one call, which nested/gated names never are.

Fixed by resetting `fairBags` only when `randomScenario` is given an EXPLICIT seed (`reseed(seed, {resetFairBags: explicitSeed})`) — a deliberate single-scenario reproduction, where purity independent of prior call history is the whole point. An unseeded call (every ordinary sweep iteration) now lets `fairBags` carry over between calls, restoring the cross-call round-robin guarantee. This doesn't weaken reproducibility: nobody replays "the sweep's own 743rd unseeded call" in isolation — they replay its own seed, which still gets a full, seed-pure reset. Verified: the "randomScenario(seed) reproducibility" suite (Commit 1's own 5 tests) still passes unchanged; the coverage-report test passed 10/10 fresh runs after the fix (previously observed failing roughly 1 run in 5).

**2. A genuine cross-year concessional-cap double-count, found by deliberately forcing the exact collision the spec's own "Bug 5" describes.** The native-cascade random generator (spec 37 Commit 1) already creates split branches and superConcessional destinations, but doesn't specifically engineer two branches sharing one target — left to chance, this exact shape is rare. Built directly instead: one step, two sibling branches (a 50/50 split), BOTH targeting the same salary-sacrifice row, with far more surplus available than the remaining concessional headroom. Year 0 correctly capped at the full concessional cap (the live-decrement `concessionalHeadroomAfterFills` pattern, already proven for a SINGLE claimant against `reserveFromSuper`'s super-account-sharing case, held for two sibling branches too). Year 1 did not: `salarySacrifice` read $62,207.32 against a $31,707.32 cap — very nearly a clean double, and the excess was EXACTLY $30,500.00, year 0's own unused-cap figure.

Root cause: `superCarryForward[p]`'s ledger update (`processConcessionalCap`, `deterministic.js`) is computed in the per-person loop BEFORE the FY-end sweep runs — needed, because the sweep's OWN cap check (`concessionalHeadroomAfterFills`) depends on that SAME year's carry-forward figure being already known. But the carry-forward computation's own `totalCC` (`otherConcessional + fillTotal`) has no visibility into what the sweep is about to add: it saw only the $2,000 ordinary contribution, concluded $30,500 of the cap went unused, and carried that $30,500 forward into next year — even though the sweep, running moments later in the SAME year's monthly loop, actually consumed the full $32,500. Next year re-used that same $30,500 of "unused" cap on top of its own genuine headroom, hence the near-double. Not a literal money leak (wcaBal/superBal both correctly reflect only what was actually paid and credited) — the leak is in the CAP-ENFORCEMENT ledger a member's NEXT year inherits.

Fixed by redoing the carry-forward computation once the real pass has actually run: a new `ccInputsByPerson` capture (this FY's pre-sweep `totalCC` and the carry-forward value it was computed against) taken right before the original (tentative) `superCarryForward[p]` assignment, then — immediately after `runYear`'s real pass returns — summing what the sweep itself credited to each person's own accounts (`row.superDetail[id].surplusSalarySacrifice`/`surplusPersonalDeductible`) and re-running `processConcessionalCap` with the corrected total, overwriting the tentative ledger value. The sweep's own in-year cap check is untouched (already correct); only the ledger NEXT year reads is corrected.

**Scope decision, on the record.** Division 293's `lowTaxContributions` and HELP/MLS's `reportableSuperContributions` (both feed `repaymentIncome`'s shared assessment, Commit 2's own fix) are computed at the SAME pre-sweep point as the carry-forward bug was, and have the identical blind spot: a surplus-cascade-sourced concessional contribution doesn't count toward either figure for the YEAR it's actually made, understating that year's Division 293/HELP/MLS income base. NOT fixed here — unlike carry-forward, these feed `assessPerson` calls that run substantially earlier in the SAME year's processing (inside the measure pass, before the sweep exists to correct against), so closing this properly means restructuring WHEN Division 293/HELP/MLS assessment happens relative to the surplus sweep, not a same-shape post-hoc correction. A materially smaller-blast-radius, genuinely separate architectural question from the conservation-critical carry-forward bug this commit exists to close; recorded here rather than either silently left or scope-crept into this already-large gate commit.

**Not a new money flow.** The surplus cascade's superConcessional destination already existed (spec 37 Commit 1, already registered); this fixes a bookkeeping defect in an EXISTING flow's cap-tracking, not a new flow. No `conservationCheck.js` extension required. The `pickFair` fix touches test infrastructure only, not the engine.

**Conservation confirmed, thoroughly, not just asserted.** The full "Conservation invariant" describe block (12 tests, including the 300-scenario gate, the 3,000-scenario stratified sweep, and the coverage report) re-run clean 6 additional times after both fixes landed, on top of the 10 additional coverage-report-only runs that verified the `pickFair` fix specifically — 16 total fresh runs, zero failures.

Tests: 1 new (the deliberate split-branch/shared-cap regression above; `git stash`-confirmed it fails with the exact $62,207.32 figure pre-fix, not a vacuous pass). Full suite 2249/2249, build green.
...
### Surplus cascade, Commit 8: splits and UI — closes spec 39

**Splits: already built, confirmed rather than rebuilt.** Spec 37 Commit 1's own split-branch behaviour ("a closed branch's own share cascades to the NEXT STEP, never redistributed to its sibling branch") is exactly what the spec's own "state what happens to a closed branch's share" asks for, already proven by an existing test. Nothing to add at the engine level; this commit's own job was giving an adviser a way to actually AUTHOR one.

**The settings UI now edits the native cascade vocabulary directly** — steps, branches, destinations, conditions — replacing the old period/allocation editor (`surplusPeriodCardHTML`, `onSurplusPeriodChange`, `onSurplusPeriodAction`, all removed). `state.settings.surplus.periods` may still legitimately hold an old-shaped period (a plan saved before this UI existed); `surplusCascadeSteps()` upgrades it via the existing `clampCascadeStep` for DISPLAY, and every edit now SAVES back step-shaped objects (`commitSurplusCascade`) — the migration Commit 5 proved bit-identical at the engine level now actually happens the first time an adviser touches this panel, closing the "do not keep two systems" instruction at the point where it's visible, not just internally. Each step is a card: destination type (debt/asset/super concessional/super non-concessional/goal/cash/expenditure), type-specific sub-fields (a loan scope + explicit multi-select for debt, the account for super, the new `allowBringForward` toggle for the NCC destination), an until-condition list (kind + amount/date, add/remove), an optional split (add/remove branches, percentage), and add/remove/reorder for steps themselves. The resolved-effect line stays (now evaluated at year 0 uniformly, since a step has no "from"/"to" of its own to anchor it the way a period did); **the year each condition is projected to be met is new** — read straight off the real projection's own per-target reporting fields (never re-derived), with one disclosed approximation: a debt condition scoped by deductibility FRACTION (rather than an explicit loan selection or "any") reads each candidate loan's whole balance, since the live deductible/non-deductible split is a dynamic engine computation this display layer has no access to.

**Four real bugs found through actual browser use, not unit tests** — `main.js` is DOM-dependent and untestable via vitest, so CLAUDE.md's own "start the dev server and use the feature" instruction is the only way any of these could have been caught before a real adviser hit them:
1. **Destination-type dropdown could show a type that didn't match what was actually stored.** A type filter that hides a type with nothing currently eligible to target (e.g. "Debt" when the plan has no liabilities) also hid the CURRENT destination's own type whenever it happened to have gone ineligible — the browser then silently selects the first remaining option, showing "Asset" selected while the sub-fields underneath were still debt's own Scope/Pay-down order controls. Fixed: the current type always stays in the list, regardless of eligibility, even though it's hidden from the list of types you could SWITCH to.
2. **Selecting a target-requiring destination type silently deleted the whole branch.** `createCascadeDestination(type)` leaves `targetId: null`; `clampCascadeDestination` treats a non-matching targetId as an invalid reference and drops it — and `clampCascadeBranch` drops the whole branch when its destination is invalid. Picking "Super (non-concessional)" (or Asset/Goal/Super concessional) with nothing yet selected silently deleted the branch on the next commit. Fixed: a new `cascadeDefaultDestination(type)` pre-selects the first eligible target, mirroring the old UI's own `surplusDefaultTarget` convention.
3. **A first split was permanently impossible.** A single branch always holds 100% by construction (its own percentage input is hidden until there's a sibling to show it against) — gating "+ Split into another branch" on leftover headroom (`usedPct < 100`) meant the button could never appear on a step's first branch, so no step could ever be split for the first time. Fixed: the button is always available; adding a branch either takes leftover headroom (if any) or splits every branch, including the new one, evenly.
4. **"+ Add step" did nothing, ever.** `onCascadeAction`'s own step lookup (`steps.findIndex(s => s.id === sid); if (si < 0) return;`) ran BEFORE the action dispatch — and the "+ Add step" button carries no `data-sid` (there's no step yet to attach one to), so every click hit the early return before ever reaching the `"add-step"` branch. Fixed: `"add-step"` is handled first, ahead of the step lookup it doesn't need.

All four were caught by literally driving the editor end-to-end in a real (Playwright-automated) browser session — add a liability, a super account, concessional and non-concessional contribution rows; open Settings; switch every destination type; toggle bring-forward; add/remove conditions; split and remove branches; add/move/remove steps — and only then trusted as working. `console --errors` clean throughout (the only console noise was this sandbox's own blocked external font/CDN requests, unrelated to the app).

**A fifth gap, found the same way, one level down.** `surplusDestinationBreakdown` (`focusSurplusAllocation.js`) — the ONE function the settings resolved-effect line, the Cashflow table's Funding group, and the Focus → Surplus allocation view all share for "where did the surplus actually go" — summed `surplusSalarySacrifice`/`surplusPersonalDeductible` but not the new `surplusNonConcessional` (Commit 7's own reporting-only field), so a surplus-cascade NCC contribution would show as if nothing happened in all three places at once. Fixed with the same one-line addition Commit 7's own field was built for.

**Engine contract.** `ENGINE_VERSION` 2.2.0 → 2.3.0 (both additive): `superNonConcessional` joins `CASCADE_DESTINATION_TYPES`, and `superDetail.<accountId>.surplusNonConcessional` joins the published per-account fields. `engineContractShape.js` and `docs/reference/engine-api.md`'s version table both updated in this commit.

**Not a new money flow.** Every fix here is UI wiring or a reporting-field read; nothing changes what the engine actually credits/debits. No `conservationCheck.js`/`randomScenario()` change needed.

Spec 39 is now closed: all eight commits landed, Commit 6 gated 7 and 8 as instructed, conservation held throughout.
