...
| `2.1.0` | Additive: yearly-row `totalAssets` (docs/specs/37-review-remediation.md, Commit 4) — real $, every balance type `netAssets` itself already sums (`closingBalance`/`propertyClosing`/`superClosing`/`pensionClosing`/`bondsClosing`/`wcaClosing`), published on its own so a display never has to re-list the components and risk omitting one. `netAssets` now derives from it (`totalAssets − liabilitiesClosing − heasDetail.closing`), so the two can never drift apart by construction. *(This row was missed at the time — added retroactively; `ENGINE_VERSION` and the contract-shape snapshot were both correctly updated in that commit, only this table lagged.)* |
| `2.2.0` | Additive: yearly-row `taxDetail.{client,partner}.helpMlsTopUpAccrued`/`helpMlsTopUpSettled`, and the household-level `taxDetail.helpMlsTopUpAccrued`/`helpMlsTopUpSettled` (docs/specs/39-cleanup-rules-cascade.md, Commit 2) — HELP/MLS were withheld same-year from an income base that excluded that FY's own net capital gain (the identical gap spec 37 Commit 5 fixed for Division 293); the gain-driven increment now defers to July of FY t+1 like CGT/Division 293/296 already do. `Accrued` is this FY's own increment (settling next year); `Settled` is last FY's increment settling as cash THIS year. |
| `2.3.0` | Additive: `schedule.surplusCascade`'s branch `destination.type` gains `superNonConcessional` (docs/specs/39-cleanup-rules-cascade.md, Commit 7) — the NCC leg of "CC to cap, then NCC" (a `superConcessional` step followed by a `superNonConcessional` one; v1 scope: `personalNonDeductible` contribution rows only), with its own `allowBringForward` flag (opt-in, default `false` — a request beyond current headroom only triggers/extends a bring-forward window when explicitly set). Yearly-row `superDetail.<accountId>.surplusNonConcessional` — real $, reporting-only (the same shape as the pre-existing `surplusSalarySacrifice`/`surplusPersonalDeductible`, needing no conservation add-back either): the slice of that account's `nonConcessional` total that arrived via this cascade destination specifically, distinct from an ordinary `personalNonDeductible` row landing in the same account. |
